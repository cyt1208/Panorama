...
void do_columbia_peak(void)
  ...
  th.push_back(thread([&](){create_panorama(im,"0-1","0","1" ,2,0,0.05,7,7,5,50000,100,1);}));
  th.push_back(thread([&](){create_panorama(im,"2-3","2","3" ,2,0,0.05,7,7,5,50000,100,1);}));
  th.push_back(thread([&](){create_panorama(im,"4-5","4","5" ,2,0,0.05,7,7,5,50000,100,1);}));
  th.push_back(thread([&](){create_panorama(im,"6-7","6","7" ,2,0,0.05,7,7,5,50000,100,0.9);}));
  th.push_back(thread([&](){create_panorama(im,"8-9","8","9" ,2,0,0.05,7,7,5,50000,100,0.9);}));
  for(auto&e1:th)e1.join();th.clear();
  
  
  th.push_back(thread([&](){create_panorama(im,"8--10",  "8-9",  "10"  ,2,0,0.15,11,7,5,50000,100,1);}));
  th.push_back(thread([&](){create_panorama(im,"0--3" ,  "0-1",  "2-3" ,2,0,0.04,11,7,5,50000,100,1);}));
  th.push_back(thread([&](){create_panorama(im,"4--7" ,  "4-5",  "6-7" ,2,0,0.04,11,7,5,50000,100,0.9);}));
  for(auto&e1:th)e1.join();th.clear();
  
  
  create_panorama(im,"4--10" , "4--7",  "8--10" ,2,0,0.04,10,7,5,50000,100,0.9);
  create_panorama(im,"all"   , "0--3",  "4--10" ,2,0,0.04,10,7,5,50000,100,0.9);
...
void do_field(void)
  ...
  th.push_back(thread([&](){create_panorama(im,"2--5", "4-5",  "2-3" ,3,0,0.05,11,7,5,50000,100,1);}));
  for(auto&e1:th)e1.join();th.clear();
  
  create_panorama(im,"all"   , "2--5",  "6-7" ,3,0,0.05,11,7,5,50000,100,1);
...
void do_helens(void)
  ...
  image_map im;
  load_images(im,indir,outdir,6,2,950);  // im, dir, numpics (0..numpics-1),  PROJ_METHOD,  FOCAL_LEN
  
  vector < thread > th; 
  
  create_panorama(im, "1-2", "1", "2", 2, 0, 0.05, 11, 7, 5, 150000, 100, 0.5);
  th.push_back(thread([&](){create_panorama(im, "0--2", "1-2", "0", 2, 0, 0.05, 11, 7, 5, 150000, 100, 0.5); }));
  th.push_back(thread([&](){create_panorama(im,"3-4","4","3" ,2,0,0.05,11,7,5,150000,100,1);}));
  //th.push_back(thread([&](){create_panorama(im,"0--2","1-2","0" ,2,0,0.05,11,7,5,150000,100,0.5);}));
  for(auto&e1:th)e1.join();th.clear();
  
  create_panorama(im,"3--5", "3-4",  "5" ,2,0,0.05,11,7,5,50000,100,0.9);
  create_panorama(im,"all" , "3--5", "0--2" ,2,0,0.05,11,7,5,50000,100,0.5);
...
void do_cse_glade(void)
  ...
  th.push_back(thread([&](){create_panorama(im,"1--8" , "5--8", "1--4" ,2,0,0.04,11,7,5,50000,100,0.5);}));
  th.push_back(thread([&](){create_panorama(im,"9--16" ,"9--12","13--16" ,2,0,0.04,11,7,5,50000,100,0.5);}));
  for(auto&e1:th)e1.join();th.clear();
  
  create_panorama(im,"1--16" , "1--8", "9--16" ,2,0,0.04,11,7,5,50000,100,0.5);
...
void do_yd(void) {
	string indir = "pano/yading/";
	string outdir = "output/yading/";

	vector < thread > th;
	image_map im;
	load_images(im, indir, outdir, 6, 0, 2000);  // im, dir, numpics (0..numpics-1),  PROJ_METHOD,  FOCAL_LEN

	//th.push_back(thread([&]() {create_panorama(im, "0-1", "0", "1", 2, 0, 0.05, 7, 7, 5, 50000, 100, 0.2); }));
	th.push_back(thread([&]() {create_panorama(im, "3-4", "4", "3", 2, 0, 0.05, 7, 7, 5, 50000, 100, 0.5); }));
	for (auto&e1 : th)e1.join(); th.clear();

	create_panorama(im, "2--4", "3-4", "2", 2, 1, 0.05, 7, 7, 7, 50000, 100, 0.5);
	create_panorama(im, "2---5", "2--4", "5", 2, 1, 0.05, 7, 7, 6, 50000, 100, 0.5);
...
void do_snow(void) {
	string indir = "pano/snow/";
	string outdir = "output/snow/";

	vector < thread > th;
	image_map im;
	load_images(im, indir, outdir, 3, 2, 2100);  // im, dir, numpics (0..numpics-1),  PROJ_METHOD,  FOCAL_LEN

	//th.push_back(thread([&]() {create_panorama(im, "0-1", "0", "1", 2, 0, 0.05, 7, 7, 5, 50000, 100, 0.2); }));
	th.push_back(thread([&]() {create_panorama(im, "0-1", "1", "0", 2, 0, 0.05, 7, 7, 3, 50000, 100, 1); }));
	for (auto&e1 : th)e1.join(); th.clear();

	create_panorama(im, "0--2", "0-1", "2", 2, 0, 0.05, 7, 7, 4, 50000, 100, 0.9);
...
void do_xiling(void) {
	string indir = "pano/xiling/";
	string outdir = "output/xiling/";

	vector < thread > th;
	image_map im;
	load_images(im, indir, outdir, 3, 1, 1100);  // im, dir, numpics (0..numpics-1),  PROJ_METHOD,  FOCAL_LEN

	//th.push_back(thread([&]() {create_panorama(im, "0-1", "0", "1", 2, 0, 0.05, 7, 7, 5, 50000, 100, 0.2); }));
	th.push_back(thread([&]() {create_panorama(im, "0-2", "0", "2", 2, 0, 0.05, 11, 7, 5, 150000, 100, 0.5); }));
	for (auto&e1 : th)e1.join(); th.clear();

	create_panorama(im, "0--2", "0-2", "1", 2, 0, 0.05, 11, 7, 5, 150000, 100, 0.5);
...
void do_my(void) {
	string indir = "pano/my/";
	string outdir = "output/my/";

	vector < thread > th;
	image_map im;
	load_images(im, indir, outdir, 8, 0, 600);  // im, dir, numpics (0..numpics-1),  PROJ_METHOD,  FOCAL_LEN

	th.push_back(thread([&]() {create_panorama(im, "0-1", "0", "1", 2, 0, 0.05, 11, 7, 5, 150000, 100, 0.9); }));
	th.push_back(thread([&]() {create_panorama(im, "4-5", "5", "4", 2, 0, 0.05, 11, 7, 5, 150000, 100, 0.9); }));
	th.push_back(thread([&]() {create_panorama(im, "2-3", "2", "3", 2, 0, 0.05, 11, 7, 5, 150000, 100, 0.9); }));
	th.push_back(thread([&]() {create_panorama(im, "6-7", "6", "7", 2, 0, 0.05, 11, 7, 5, 150000, 100, 0.9); }));
	for (auto&e1 : th)e1.join(); th.clear();

	th.push_back(thread([&]() {create_panorama(im, "0---3", "0-1", "2-3", 2, 0, 0.05, 11, 7, 5, 150000, 100, 0.9); }));
	th.push_back(thread([&]() {create_panorama(im, "4---7", "6-7", "4-5", 2, 0, 0.05, 11, 7, 5, 150000, 100, 0.9); }));
	for (auto&e1 : th)e1.join(); th.clear();

	create_panorama(im, "all", "0---3", "4---7", 2, 0, 0.05, 11, 7, 5, 150000, 100, 0.7);
...
int main(int argc, char **argv)
  ...
  if(string(argv[1]) == "yading")do_yd();
  if (string(argv[1]) == "snow")do_snow();
  if (string(argv[1]) == "xiling")do_xiling();
