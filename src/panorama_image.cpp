...
vector<int> match_descriptors_a2b(const vector<Descriptor>& a, const vector<Descriptor>& b)
  ...
  for(int j=0;j<(int)a.size();j++)
	...
	Descriptor a_des = a.at(j);
	for (int i = 0; i < (int)b.size(); i++) {
	  float distance = l1_distance(a_des.data, b[i].data);
...
vector<Match> match_descriptors(const vector<Descriptor>& a, const vector<Descriptor>& b)
  ...
  for (int i = 0; i < (int)a.size(); i++) {
	int b_match = a2b.at(i);
	if (b2a.at(b_match) == i) {
	  float dist = l1_distance(a.at(i).data, b.at(b_match).data);
	  Match match = Match(&a[i], &b[b_match], dist);
	  m.push_back(match);
	}
  }
  return m;
...
Point project_point(const Matrix& H, const Point& p)
  ...
  p_origin(1) = p.y;
  p_origin(2) = 1;
  Matrix p_new = H * p_origin;
  double x = p_new(0) / p_new(2);
  double y = p_new(1) / p_new(2);
	...
	double h21 = H(2, 1);
	double h20 = H(2, 0);
	double x_new = ((1 + h00)*p.x + h01 * p.y + h02) / (h20*p.x + h21 * p.y + 1);
	double y_new = (h10*p.x + (1 + h11)*p.y + h12) / (h20*p.x + h21 * p.y + 1);
...
vector<Match> model_inliers(const Matrix& H, const vector<Match>& m, float thresh)
  ...
  for (int i = 0; i < (int)m.size(); i++) {
	Point a_projected = project_point(H, m[i].a->p);
	if (point_distance(a_projected, m[i].b->p) < thresh)
...
Matrix compute_homography_ba(const vector<Match>& matches)
{
	...
	Matrix b(matches.size() * 2);
		...
		for (int i = 0; i < (int)matches.size(); ++i)
		{
			...
			M(i * 2, 0) = M(i * 2 + 1, 3) = mx;
			M(i * 2 + 1, 0) = M(i * 2 + 1, 1) = M(i * 2 + 1, 2) = M(i * 2, 3) = M(i * 2, 4) = M(i * 2, 5) = 0;
			M(i * 2, 1) = M(i * 2 + 1, 4) = my;
			M(i * 2, 2) = M(i * 2 + 1, 5) = 1;
			M(i * 2, 6) = -mx * nx;
			M(i * 2 + 1, 6) = -mx * ny;
			M(i * 2, 7) = -my * nx;
...
Matrix compute_translation(const vector<Match>& matches) {
	Matrix projection(3, 3);
	projection(0, 0) = projection(1, 1) = projection(2, 2) = 1;
	projection(0, 1) = projection(1, 0) = projection(2, 0) = projection(2, 1) = 0;
	double tx = 0;
	double ty = 0;
	for (int i = 0; i < matches.size(); i++) {
	  tx += matches[i].b->p.x - matches[i].a->p.x;
	  ty += matches[i].b->p.y - matches[i].a->p.y;
	}
	projection(0, 2) = tx / (matches.size()*1.0);
	projection(1, 2) = ty / (matches.size()*1.0);

	return projection;
}

Matrix compute_affine(const vector<Match>& matches) {
	Matrix M(matches.size() * 2, 6);
	Matrix b(matches.size() * 2);
	...
	for (int i = 0; i < matches.size(); i++) {
		...
		M(2 * i, 0) = M(2 * i + 1, 3) = mx;
		M(2 * i, 1) = M(2 * i + 1, 4) = my;
		M(2 * i, 2) = M(i * 2 + 1, 5) = 1;
		M(2 * i, 3) = M(2 * i, 4) = M(2 * i, 5) = M(2 * i + 1, 0) = M(2 * i + 1, 1) = M(2 * i + 1, 2) = 0;
...
Matrix RANSAC(vector<Match> m, float thresh, int k, int cutoff)
  ...
  for (int iter = 0; iter < k; iter++) {
	int N = 4;
	randomize_matches(m);
	vector<Match> samples(m.begin(), m.begin() + N);
	Matrix temp = compute_homography_ba(samples);
	vector<Match> inliers = model_inliers(temp, m, thresh);
	Matrix tran = compute_translation(samples);
	vector<Match> inliers_tran = model_inliers(tran, m, thresh);
	Matrix affine = compute_affine(samples);
	vector<Match> inliers_aff = model_inliers(affine, m, thresh);
	if (inliers.size() > best && inliers.size() > inliers_aff.size() && inliers.size() > inliers_tran.size()) {
		Hba = temp;
		best = inliers.size();
	  Matrix new_Hba = compute_homography_ba(inliers);
	  vector<Match> performance = model_inliers(new_Hba, m, thresh);
	  if (performance.size() > best) {
		  best = performance.size();
		  Hba = new_Hba;
	  }
	}
	else if (inliers_aff.size() > best && inliers_aff.size() >= inliers.size() && inliers_aff.size() > inliers_tran.size()) {
		Hba = temp;
		best = inliers.size();
		Matrix new_Hba = compute_affine(inliers);
		vector<Match> performance = model_inliers(new_Hba, m, thresh);
		if (performance.size() > best) {
			best = performance.size();
			Hba = new_Hba;
	...
	else if (inliers_tran.size() > best && inliers_tran.size() >= inliers_aff.size() && inliers_tran.size() >= inliers.size()) {
		Hba = temp;
		best = inliers.size();
		Matrix new_Hba = compute_translation(inliers);
		vector<Match> performance = model_inliers(new_Hba, m, thresh);
		if (performance.size() > best) {
			best = performance.size();
			Hba = new_Hba;
  ...
  //Point test = project_point(Hba, Point(0, 0));
...
/*Matrix RANSAC(vector<Match> m, float thresh, int k, int cutoff)
{
	if (m.size() < 4)
	{
		//printf("Need at least 3 points for RANSAC! %zu supplied\n",m.size());
		return Matrix::identity(3, 3);
	}
...
Image combine_images(const Image& a, const Image& b, const Matrix& Hba, float ablendcoeff)
  ...
  // TODO: Put your code here.
  int start_x = topleft.x - dx;
  int start_y = topleft.y - dy;
  int end_x = botright.x - dx;
  int end_y = botright.y - dy;
  for (int k = 0; k < c.c; k++) {
	  for (int i = start_x; i < end_x; i++) {
		  for (int j = start_y; j < end_y; j++) {
			  ...
			  float value_b = 0;
			  if (b.is_nonempty_patch(pb.x, pb.y, 1) && pb.x >= 0 && pb.x < b.w && pb.y >= 0 && pb.y < b.h) {
				  value_b = b.pixel_bilinear(pb.x, pb.y, k);
			  }
			  float value_a = 0;
			  if (i + dx >= 0 && i + dx < a.w && j + dy >= 0 && j + dy < a.h && a.is_nonempty_patch(i + dx, j + dy, 0)) {
				  value_a = a(i + dx, j + dy, k);
				  if (value_b) {
					  c(i, j, k) = value_a * ablendcoeff + value_b * (1 - ablendcoeff);
					  continue;
				  }
			  }
			  c(i, j, k) = value_a + value_b;
		  }
	  }
  }
...
Image cylindrical_project(const Image& im, float f)
  ...
  //Image c(2 * hfov*f, 2 * vfov*f, im.c);
  double r = c.h / (2*vfov);
  for (int k = 0; k < c.c; k++) {
	for (int i = 0; i < c.w; i++) {
	  for (int j = 0; j < c.h; j++) {
		  double x = x_c + f * tan(i/r - x_c / f);
		  double y = (j - y_c*r/f)*sqrt(pow(x - x_c*r/f, 2) + f * f) / r + y_c;
...
Image spherical_project(const Image& im, float f)
  ...
  for (int k = 0; k < c.c; k++) {
	  for (int i = 0; i < c.w; i++) {
		  for (int j = 0; j < c.h; j++) {
			  double x = f * tan(i / r - x_c / f) + x_c;
			  double y = tan(j / r - y_c / f)*f*sqrt(1 + pow(tan(i / r - x_c / f), 2))+y_c;
			  if (x < 0 || x >= im.w || y < 0 || y >= im.h) {
				  c(i, j, k) = 0;
				  continue;
			  }
